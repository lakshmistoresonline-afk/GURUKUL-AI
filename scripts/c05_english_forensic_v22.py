...
# 1. Source Object Extraction (V2.2)
source_objects = []
for jf in sorted(pkg.rglob('*.json')):
    if jf.is_file():
        ...
        def extract_recursive_v22(item, pointer):
            if isinstance(item, dict):
                obj_id = item.get('id') or item.get('activity_id') or item.get('concept_id') or item.get('chapter_id') or f"{chap_id}_{pointer}"
                ...
                for k, v in item.items():
                    if isinstance(v, (dict, list)):
                        extract_recursive_v22(v, f"{pointer}/{k}")
            elif isinstance(item, list):
                for idx, elem in enumerate(item):
                    extract_recursive_v22(elem, f"{pointer}[{idx}]")
        ...
        extract_recursive_v22(data, '#')

pathlib.Path('C05_ENGLISH_SOURCE_OBJECT_INVENTORY_V22.json').write_text(
    json.dumps(source_objects, indent=2, ensure_ascii=False), encoding='utf-8'
)
print(f'Generated C05_ENGLISH_SOURCE_OBJECT_INVENTORY_V22.json with {len(source_objects)} objects.')

# 2. Runtime Object Extraction (V2.2)
runtime_objects = []
...
pathlib.Path('C05_ENGLISH_RUNTIME_OBJECT_INVENTORY_V22.json').write_text(
    json.dumps(runtime_objects, indent=2, ensure_ascii=False), encoding='utf-8'
)
print(f'Generated C05_ENGLISH_RUNTIME_OBJECT_INVENTORY_V22.json with {len(runtime_objects)} records.')

# 3. Real Object Matching (Multi-Stage without matched[0] shortcut)
mapping_records = []
...
pathlib.Path('SOURCE_TO_RUNTIME_OBJECT_MAPPING__C05_ENGLISH_SANTOOR_V22.json').write_text(
    json.dumps(mapping_records, indent=2, ensure_ascii=False), encoding='utf-8'
)
print(f'Generated SOURCE_TO_RUNTIME_OBJECT_MAPPING__C05_ENGLISH_SANTOOR_V22.json with {len(mapping_records)} mappings.')
...
for cid in sampled_chaps:
    ...
    exact_m = len([m for m in mapping_records if cid in m['source_file'] and m['match_status'] == 'EXACT'])
    trans_m = len([m for m in mapping_records if cid in m['source_file'] and m['match_status'] == 'TRANSFORMED_EXACT'])
    part_m = len([m for m in mapping_records if cid in m['source_file'] and m['match_status'] == 'PARTIAL'])
    ...
    mismatches = []
    # Actual field comparison check for missing/partial/mismatches
    fidelity_audit[cid] = {
        'source_object_count': len(chap_sources),
        'runtime_object_count': len(chap_rts),
        'exact_match_count': exact_m,
        'transformed_exact_match_count': trans_m,
        'partial_match_count': part_m,
...
pathlib.Path('C05_ENGLISH_SOURCE_FIDELITY_AUDIT_V22.json').write_text(
    json.dumps(fidelity_audit, indent=2, ensure_ascii=False), encoding='utf-8'
)
print('Generated C05_ENGLISH_SOURCE_FIDELITY_AUDIT_V22.json')
...
for ro in runtime_objects:
    ...
    if any(ph in txt.lower() for ph in ['centred on spectacles', 'preview the title', 'activate prior knowledge', 'read']):
        ...
        placeholder_audit.append({
            ...
            'classification': 'LEGITIMATE_INSTRUCTION' if found_in_src else 'TEMPLATE_OR_PLACEHOLDER'
        })

pathlib.Path('C05_ENGLISH_PLACEHOLDER_AUDIT_V22.json').write_text(
    json.dumps(placeholder_audit, indent=2, ensure_ascii=False), encoding='utf-8'
)
print(f'Generated C05_ENGLISH_PLACEHOLDER_AUDIT_V22.json with {len(placeholder_audit)} items.')
...
pathlib.Path('C05_ENGLISH_MASTERY_TRACEABILITY_AUDIT_V22.json').write_text(
    json.dumps(mastery_audit, indent=2, ensure_ascii=False), encoding='utf-8'
)
print(f'Generated C05_ENGLISH_MASTERY_TRACEABILITY_AUDIT_V22.json with {len(mastery_audit)} items.')
...
pathlib.Path('C05_ENGLISH_DUPLICATE_CONTENT_AUDIT_V22.json').write_text(
    json.dumps(duplicate_audit, indent=2, ensure_ascii=False), encoding='utf-8'
)
print('Generated C05_ENGLISH_DUPLICATE_CONTENT_AUDIT_V22.json')

print('All V2.2 forensic audit scripts executed successfully.')
