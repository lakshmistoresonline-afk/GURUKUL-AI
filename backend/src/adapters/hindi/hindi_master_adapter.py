...
class HindiMasterAdapter(BaseContentAdapter):
    """
    Adapter for Hindi Master & Notes Datasets (Class 5, 6, 7).
    Maps all Hindi JSON datasets losslessly (overview, pedagogical objectives, stanza-wise explanations,
    character/element sketches, exhaustive vocabulary, synonyms, antonyms, spelling corrections,
    comprehensive grammar including nouns, pronouns, adjectives, verbs, case markers, gender/number, idioms,
    comprehension extracts, question bank, model papers, flashcards, story mindmap, interactive quiz).
    """
    ...
    def parse_chapter(
        ...
        # 1. OVERVIEW STAGE — Summary, Theme & Pedagogical Objectives
        ...
        # 2. LEARN STAGE — Stanza-wise Explanations & Poetic Analysis
        ...
        if stanzas:
            ...
            block = processor.process(
                block_id=f"{chapter_id}-stanzas-{order}",
        ...
        # 3. LEARN STAGE — Shabdart / Terminology (शब्दार्थ)
        ...
        if meanings:
            ...
            block = processor.process(
                block_id=f"{chapter_id}-shabdart-{order}",
        ...
        # 4. LEARN STAGE — Shuddhi Vartani (शुद्धि-वर्तनी)
        ...
        if vartani:
            ...
            block = processor.process(
                block_id=f"{chapter_id}-shuddhi_vartani-{order}",
        ...
        # 5. LEARN STAGE — Character & Element Sketches (पात्र परिचय)
        chars = source_data.get("character_analysis") or source_data.get("character_and_element_sketches") or []
        if chars:
            processor = ProcessorRegistry.get_processor("detailedBreakdown")
            block = processor.process(
                block_id=f"{chapter_id}-character_analysis-{order}",
        ...
        # 6. LEARN STAGE — Comprehensive Grammar (व्याकरण एवं भाषा ज्ञान)
        grammar_obj = source_data.get("comprehensive_grammar") or source_data.get("grammar_extraction") or {}
        if grammar_obj:
            processor = ProcessorRegistry.get_processor("detailedBreakdown")
            block = processor.process(
                block_id=f"{chapter_id}-comprehensive_grammar-{order}",
        ...
        # 7. LEARN STAGE — Comprehension Extracts (पाठ परिचय एवं गद्यांश/पद्यांश)
        extracts = source_data.get("comprehension_and_extracts") or []
        if extracts:
            processor = ProcessorRegistry.get_processor("detailedBreakdown")
            block = processor.process(
                block_id=f"{chapter_id}-comprehension_and_extracts-{order}",
        ...
        if acts:
            processor = ProcessorRegistry.get_processor("detailedBreakdown")
            block = processor.process(
                block_id=f"{chapter_id}-activities_and_projects-{order}",
        ...
        block = processor.process(
            block_id=f"{chapter_id}-question_bank-{order}",
        ...
        if model_paper:
            ...
            block = processor.process(
                block_id=f"{chapter_id}-model_question_paper-{order}",
        ...
        if mindmap_data:
            ...
            block = processor.process(
                block_id=f"{chapter_id}-story_mindmap-{order}",
                source_type="mindmap",
        ...
        if quiz_data:
            ...
            block = processor.process(
                block_id=f"{chapter_id}-interactive_quiz-{order}",
                source_type="quiz",
                raw_data=quiz_data,
