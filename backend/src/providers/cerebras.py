...
class CerebrasProvider(AIProvider):
    ...
    def is_enabled(self) -> bool:
        return settings.CEREBRAS_ENABLED and self.api_key is not None
    ...
    async def generate_structured(self, prompt: str, schema: Dict[str, Any], model_override: Optional[str] = None) -> Dict[str, Any]:
        ...
        # Cerebras might not support response_format: json yet, so we use prompt instruction
        payload = {
            "model": model_override or self.model,
            "messages": [
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                raise Exception(f"Cerebras structured error: {response.status_code} - {response.text}")

    async def generate_embeddings(self, text: str) -> List[float]:
        raise NotImplementedError("Cerebras does not support embeddings yet")
