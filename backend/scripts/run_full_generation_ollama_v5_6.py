...
def find_target(job_dirs, class_name, chapter_id, chapter_title):
    # JOB.json uses class_name="Class 7"; subject_name identifies Hindi or
    # Social Science. Do not treat "Class 7 Hindi" as the class_name field.
    cn = _norm(class_name)
    ...
    if "hindi" in cn:
        subject_tokens = ("hindi",)
    elif "socialscience" in cn:
        subject_tokens = ("socialscience", "socialstudies")
    else:
        subject_tokens = ()
    ...
    for directory in job_dirs:
        ...
        actual_class = _norm(chapter.get("class_name"))
        actual_id = _norm(chapter.get("chapter_id"))
        actual_title = _norm(chapter.get("chapter_title"))
        subject = _norm(chapter.get("subject_name"))
        rel_path = _norm(chapter.get("rel_path"))
        combined = subject + " " + rel_path

        if actual_class != wanted_class or actual_id != wanted_id:
            continue
        if subject_tokens and not any(t in combined for t in subject_tokens):
            continue
        candidates.append((directory, job, actual_title))

    for directory, job, actual_title in candidates:
        if wanted_title and actual_title == wanted_title:
    ...
    print("[DIAGNOSTIC] Requested:", class_name, "| ID:", chapter_id,
          "| TITLE:", chapter_title)
    print("[DIAGNOSTIC] Matching class/ID/subject candidates:")
    for directory, job, _ in candidates:
        c = job["chapter"]
    ...
    if not candidates:
        print("[DIAGNOSTIC] All staged jobs with matching numeric ID:")
        for directory in job_dirs:
            try:
                job = read_json(directory / "JOB.json")
            except Exception:
            ...
            if _norm(c.get("chapter_id")) == wanted_id:
                print("  ID=%r | CLASS=%r | SUBJECT=%r | TITLE=%r | DIR=%r" %
                      (c.get("chapter_id"), c.get("class_name"),
                       c.get("subject_name"), c.get("chapter_title"),
                       directory.name))

    raise FileNotFoundError(
        "Target job not found safely: %s / %s / %s" %
...
def recover(directory, job, schema):
    ...
    chapter = job["chapter"]
    source = source_text(Path(chapter["path"]))

    if not source:
        raise RuntimeError(
            "No readable source files found for %s"
            % chapter["chapter_title"]
        )

    prompt = (
        "You are generating the LEARN pillar for an educational content "
        "repository.\n"
        "Return ONLY valid JSON with this exact outer shape:\n"
        '{"items":[{"title":"...","explanation":"..."}]}\n'
        "Every item MUST contain a non-empty title and a non-empty "
        "explanation.\n"
        "Create 2 to 5 concise, high-quality teaching items.\n"
        "The title must be a specific concept/topic actually taught by "
        "the supplied source.\n"
        "The explanation must teach that concept accurately and clearly.\n"
        "Use ONLY information supported by the supplied source.\n"
        "Do not mention the source, prompt, model, generation, metadata, "
        "instructions, or chapter-production process.\n"
        "Do not use generic titles such as 'Understanding this chapter'.\n"
        "For a chapter whose title is only an identifier such as '109_', "
        "derive meaningful teaching titles from the actual source.\n\n"
        "CLASS: " + str(chapter["class_name"]) + "\n"
        "SUBJECT: " + str(chapter["subject_name"]) + "\n"
        "CHAPTER ID: " + str(chapter["chapter_id"]) + "\n"
        "CHAPTER TITLE: " + str(chapter["chapter_title"]) + "\n\n"
        "SOURCE MATERIAL:\n" + source
    )
    ...
    for attempt in range(1, 4):
        try:
            print("    Ollama attempt %d/3" % attempt, flush=True)
            items = parse_items(ollama(prompt))
            clean = validate_learn(items, schema, job)
    ...
    raise RuntimeError("three recovery attempts failed: %s" % last)
