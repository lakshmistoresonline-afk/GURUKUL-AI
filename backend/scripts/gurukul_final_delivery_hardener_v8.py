#!/usr/bin/env python3
"""
GURUKUL AI — FINAL DELIVERY HARDENER v8
=======================================
...
OUTPUT = ROOT / "Contents_HARDENED_FINAL_V8"
REPORT = ROOT / "FINAL_CONTENTS_HARDENING_V8_REPORT.json"
AUDIT = ROOT / "FINAL_CONTENTS_HARDENING_V8_AUDIT.md"
...
# Canonical trees contain both 3-digit chapter IDs and longer numeric
# package/chapter IDs (the forensic audit found 20 Class 6/7 roots of this
# form). Do not truncate IDs.
CHAPTER_RE = re.compile(r"^(?:chapter[_\\s-]*)?(\\d{3,6})[_-](.+)$", re.I)
...
def find_chapter_roots():
    ...
    for info in SOURCE.rglob("CHAPTER_INFO.json"):
        ...
        data = load_json(info)
        metadata = metadata_identity(data)
        mid = metadata.get("chapter_id")
        mtitle = metadata.get("chapter_title")
...
def find_fallback_chapters(found):
    ...
    for class_dir in SOURCE.iterdir():
        ...
        for p in class_dir.rglob("*"):
            ...
            has_info = any(
                x.name.upper() == "CHAPTER_INFO.JSON"
...
def main():
    print("="*78)
    print("GURUKUL AI — FINAL DELIVERY HARDENER v8")
    print("CHAPTER-INFO + CONTENT-ROOT ANCHORED + 3-6 DIGIT IDS + EXACT 183 GATE")
    print("="*78)
    ...
    report = {
        "schema_version":"GURUKUL_FINAL_DELIVERY_HARDENER_V8",
        "started_at":now(),
        "source":str(SOURCE),
        "output":str(OUTPUT),
    ...
    manifest = {
        "schema_version":"GURUKUL_CANONICAL_CHAPTER_MANIFEST_V8",
        "generated_at":now(),
        "expected_total":183,
        "chapters":write_manifest(chapters)
    }
    (OUTPUT/"_GURUKUL_CANONICAL_CHAPTER_MANIFEST_V8.json").write_text(
    ...
    print("Chapter ID rule: 3-6 digit numeric prefix; IDs are never truncated.")
