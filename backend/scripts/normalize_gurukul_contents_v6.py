...
def _v6_banner():
    print("GURUKUL AI CONTENTS HARDENING v6")
    print("PATH-FIRST + PACKAGE-AWARE + 183-CHAPTER HARD GATE")
    print("Source: D:\\GURUKUL-AI\\Contents")
...
def main():
    ...
    for key in sorted(chapter_resources, key=lambda k: tuple("" if v is None else str(v) for v in k)):
        if len(key) == 4:
            cls, package_id, subject, chapter = key
        else:
    ...
    gates = {
        ...
        "class_coverage": all(
            canonical_inventory_counts.get(k, 0) == v
    ...
    for cls in ("class_5", "class_6", "class_7"):
        lines.append(
            f"- {cls}: {canonical_inventory_counts.get(cls, 0)} "
            f"/ {expected[cls]}"
        )
    ...
    print(f"Class 5      : {canonical_inventory_counts.get("class_5", 0)} / 47")
    print(f"Class 6      : {canonical_inventory_counts.get("class_6", 0)} / 64")
    print(f"Class 7      : {canonical_inventory_counts.get("class_7", 0)} / 72")
    print(f"Package collisions: {len(package_collisions)}")
