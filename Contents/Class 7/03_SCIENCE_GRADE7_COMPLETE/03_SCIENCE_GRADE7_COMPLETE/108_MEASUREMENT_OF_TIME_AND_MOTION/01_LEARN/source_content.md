...
the winner even when two sprinters seemed to cross the ﬁ nish 
...
floated on the surface of water (Fig. 8.2b). 
It gradually filled up in a fixed time and 
finally sank. Then, it was lifted up and 
floated again. 
...
second and falls on a scale ﬁ nely marked to measure time intervals as 
...
ﬂ owing out was described in the Arthasastra, Sardulakarnavadana, 
and some other texts (early CE centuries). These clocks were not very 
accurate because as water levels dropped, the ﬂ ow rate decreased. 
This led to the development of the sinking bowl water clock 
(Fig. 8.2b), or Ghatika-yantra, ﬁ rst mentioned by Aryabhata, and then 
...
each swing. After experimenting with diﬀ erent pendulums, Galileo 
...
The pendulum is said to have completed one oscillation 
when its bob, starting from its mean position O, moves to extreme 
...
You just did an important historical experiment which was ﬁ rst done 
...
or three diﬀ erent lengths. Does the time period change? If so, how does 
the length aﬀ ect it? If changing the length inﬂ uences the time period, 
does the bob’s mass also matter? Test this by repeating Activity 8.2 
with a ﬁ xed pendulum length but with bobs of diﬀ erent mass. Do you 
observe any change?
...
speciﬁ c atoms (atomic clocks). While Huygens’ early pendulum 
...
that it took 24 minutes to ﬁ ll and sink. The time unit measured by this 
...
Solution: Speed of the bicycle = Distance covered
Time taken
...
Also, ﬁ nd the distance to that station as given in the timetable. 
Note the time at which the train departs from your station and 
arrives at the next station. Find the diﬀ erence to calculate the 
...
Repeat for 4–5 diﬀ erent types of trains (Passenger/ Express/ 
Superfast).
...
We can write this equation in a diﬀ erent form to calculate the 
...
Solution: Time taken by the train = Distance covered
Speed
                                                             = 
360km
90 km
...
How were their motion diﬀ erent?
...
ﬁ rst station A at a slow speed, then moves at a faster speed, then 
...
is also ﬁ tted in the vehicles that 
...
we seldom ﬁ nd objects moving with a constant speed over long 
...
traﬃ  c and a car moving in city traﬃ  c.
7. Data for an object covering distances in diﬀ erent intervals 
of time are given in the following table. If the object is in 
uniform motion, ﬁ ll in the gaps in the table.
Time (s)
0
10
20
...
Measurement of Time and Motion
119
8. A car covers 60 km in the ﬁ rst hour, 70 km in the second 
...
of 2 km. In the ﬁ rst 500 m, it moves with a speed of 10 m/s 
...
Construct a ﬂ oating bowl-type water clock. Experiment by 
using bowls of diﬀ erent sizes and making holes of diﬀ erent 
...
What might be the reasons for the slight diﬀ erences in the 
time periods of a pendulum of a given length in diﬀ erent 
readings taken in Activity 8.2. Think of ways to control those 
and repeat the activity to check if the diﬀ erence in readings 
is reduced. 
Visit a playground with a few swings. Measure the time taken 
by a swing for 10 oscillations and calculate its time period. 
Repeat it a few times with children of diﬀ erent weights to 
ﬁ nd out if its time period is almost the same. Repeat this with 
swings of diﬀ erent lengths. Find out how the time period 
