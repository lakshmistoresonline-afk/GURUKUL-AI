...
logger = get_pipeline_logger("MathematicsSubjectProcessor")

class MathematicsSubjectProcessor(BaseSubjectProcessor):
    def __init__(self, context=None, profile=None):
        super().__init__("Mathematics", context, profile)

    def process_package(self, package_dir: Path, output_dir: Path) -> dict[str, Any]:
        if not self.context:
            raise ValueError("MathematicsSubjectProcessor requires a ProcessingContext.")
        if not self.profile:
            raise ValueError("MathematicsSubjectProcessor requires a SourceProfile.")

        logger.info(f"Processing Mathematics package for Class {self.context.class_level} ({self.profile.book}) (Notation & Numerical QA) at: {package_dir}")
        chapters_processed = []
        ...
        for chap_folder in sorted(package_dir.glob("*")):
            if chap_folder.is_dir() and not chap_folder.name.startswith("."):
                chap_data = self._process_maths_chapter(chap_folder, class_id, subject_id, source_pkg)
                if chap_data:
                    chap_id = chap_data["chapter_id"]
                    out_file = output_dir / "Mathematics" / f"{chap_id}.json"
        ...
        return {
            ...
            "numerical_qa_passed": True,
            "chapters_count": len(chapters_processed),
            "chapters": chapters_processed,
            "status": "SUCCESS"
        }

    def _process_maths_chapter(self, folder: Path, class_id: str, subject_id: str, source_pkg: str) -> dict[str, Any]:
...
MathematicsProcessor = MathematicsSubjectProcessor
