...
logger = get_pipeline_logger("EVSProcessor")

class EVSProcessor(BaseSubjectProcessor):
    def __init__(self):
        super().__init__("EVS", "5")

    def process_package(self, package_dir: Path, output_dir: Path) -> dict[str, Any]:
        logger.info(f"Processing EVS package at: {package_dir}")
        chapters_processed = []

        for chap_folder in sorted(package_dir.glob("*")):
            if chap_folder.is_dir() and not chap_folder.name.startswith("."):
                chap_data = self._process_evs_chapter(chap_folder)
                if chap_data:
    ...
    def _process_evs_chapter(self, folder: Path) -> dict[str, Any] | None:
        ...
        if source_pages.exists():
            ...
            for p in p_data.get("pages", []):
                ...
                if txt:
                    learn_recs.append({
                        "record_id": f"class_5_03_evs_complete_{chap_id}_learn_01_LESSONS_{p.get('source_page', 1)}_{calculate_text_hash(txt)}",
                        "type": "lesson",
                        "text": txt,
        ...
        return {
            "id": f"class_5_03_evs_complete_{chap_id}",
            "classId": "class_5",
            "subjectId": "03_evs_complete",
            "chapter_id": chap_id,
            "title": title,
            "learn": learn_recs,
            "practice": [],
