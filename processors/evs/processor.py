...
logger = get_pipeline_logger("EVSSubjectProcessor")

class EVSSubjectProcessor(BaseSubjectProcessor):
    def __init__(self, context=None, profile=None):
        super().__init__("EVS", context, profile)

    def process_package(self, package_dir: Path, output_dir: Path) -> dict[str, Any]:
        if not self.context:
            raise ValueError("EVSSubjectProcessor requires a ProcessingContext.")
        if not self.profile:
            raise ValueError("EVSSubjectProcessor requires a SourceProfile.")

        logger.info(f"Processing EVS package for Class {self.context.class_level} ({self.profile.book}) at: {package_dir}")
        chapters_processed = []
        ...
        for chap_folder in sorted(package_dir.glob("*")):
            if chap_folder.is_dir() and not chap_folder.name.startswith("."):
                chap_data = self._process_evs_chapter(chap_folder, class_id, subject_id, source_pkg)
                if chap_data:
                    chap_id = chap_data["chapter_id"]
                    out_file = output_dir / "EVS" / f"{chap_id}.json"
    ...
    def _process_evs_chapter(self, folder: Path, class_id: str, subject_id: str, source_pkg: str) -> dict[str, Any]:
...
EVSProcessor = EVSSubjectProcessor
