...
async def log_requests(request: Request, call_next):
    ...
    try:
        ...
        logger.info(f"RID: {request.method} {request.url.path} - STATUS: {response.status_code} - ORIGIN: {origin} - TIME: {process_time:.4f}s")
        return response
    except Exception as e:
        process_time = time.time() - start_time
        logger.error(f"RID: {request.method} {request.url.path} - CRASHED - ORIGIN: {origin} - TIME: {process_time:.4f}s - {e}", exc_info=True)
        raise
