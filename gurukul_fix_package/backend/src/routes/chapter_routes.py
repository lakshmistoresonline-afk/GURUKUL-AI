            ...
            if os.path.exists(package_path):
                try:
                    with open(package_path, "r", encoding="utf-8") as f:
                        data = json.load(f)
                        return PackageAdapter.adapt(data)
                except Exception as e:
                    logger.error(f"Error reading or adapting master package: {e}", exc_info=True)
                    # Return raw if adapt fails
                    try:
                        with open(package_path, "r", encoding="utf-8") as f: return json.load(f)
                    except: pass
