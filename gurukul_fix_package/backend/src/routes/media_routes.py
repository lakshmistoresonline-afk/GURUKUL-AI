            ...
            if os.path.exists(pkg_path):
                try:
                    with open(pkg_path, 'r', encoding='utf-8') as f:
                        ...
                        m_list = storyboard.get("resources") or pkg.get("content", {}).get("multimedia") or pkg.get("multimedia")
                        if m_list and isinstance(m_list, list):
                            logger.info(f"Media: Found {len(m_list)} bundled items in package")
                            for item in m_list:
                            ...
                            logger.info(f"Media: No bundled items found in package for {chapter_id}")
