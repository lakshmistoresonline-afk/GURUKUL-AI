        ...
        if chapterId:
            try:
                ...
                if config:
                    ...
                    if len(all_qs) < count:
                        package_cfg = mastery_service.get_chapter_mastery_config(authorized_class, subject or "", chapterId)
                        if package_cfg and "content" in package_cfg:
                             pkg_qs = package_cfg["content"].get("quiz") or []
                             all_qs.extend(pkg_qs)

                        if len(all_qs) < count and package_cfg and "original_data" in package_cfg:
                             orig_qs = package_cfg.get("original_data", {}).get("assessment", {}).get("expandedQuestionBank", [])
                             all_qs.extend(orig_qs)

                    if all_qs:
                        # Remove duplicates by ID and ensure they are dicts
                        unique_qs = {}
                        for q in all_qs:
                            if not isinstance(q, dict): continue
                            qid = str(q.get('id') or q.get('question', '')[:50])
                    ...
                    logger.warning(f"Quiz: No valid questions found for {chapterId}")
            except Exception as e:
