  ...
  useEffect(() => {
    ...
    const fetchSecondary = async () => {
      try {
        ...
        if (results[0].status === 'fulfilled') setGurukulMedia(results[0].value || []);
        if (results[1].status === 'fulfilled') setResources(results[1].value || []);
        if (results[2].status === 'fulfilled') setExternalResources(results[2].value || []);

      } catch (error: any) {
        if (error.isCanceled || axios.isCancel(error)) return;
